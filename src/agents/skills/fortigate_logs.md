...
- **Local log retrieval tool names do not contain the word "log"** (`fgt74_cmdb_disk_get_*` retrieves from `/api/v2/log/disk/...`). Never conclude from tool names or search misses that local log retrieval does not exist.
...
|---|---|
| `fgt74_cmdb_log_get_device_state` | **Always first.** Which log storage backends are enabled/available on this device (disk, memory, FortiAnalyzer, FortiCloud). Decides the rest of the investigation. |
| `fgt74_cmdb_faz_get_fortianalyzer_type` | Historical log entries from FortiAnalyzer by category (`type=webfilter`, `traffic`, `event`, ...). Variants: `fgt74_cmdb_faz_get_fortianalyzer_traffic_subtype`, `..._event_subtype`, and `..._raw` forms. |
| `fgt74_cmdb_fcloud_get_forticloud_type` | Historical log entries from FortiCloud by category. Same subtype/`_raw` variants (`fgt74_cmdb_fcloud_get_forticloud_traffic_subtype`, ...). |
| `fgt74_cmdb_disk_get_type` | Historical log entries from the local disk by category — only when device_state shows disk logging available. Variants: `fgt74_cmdb_disk_get_traffic_subtype`, `fgt74_cmdb_disk_get_event_subtype`, and `..._raw` forms. |
| `fgt74_log_mem_get_memory_type` | Log entries from local memory — the always-present fallback, short volatile window. Variants: `fgt74_log_mem_get_memory_traffic_subtype`, `..._event_subtype`, `..._raw` forms. |
| `fgt74_cmdb_log_get_fortianalyzer` / `fgt74_cmdb_log_get_fortianalyzer_queue` | FortiAnalyzer logging state and transmit queue — diagnoses a 424/unreachable backend. |
| `fgt74_cmdb_log_get_forticloud` / `fgt74_cmdb_log_get_forticloud_connection` | FortiCloud logging state and connection status — diagnoses a disabled/failing cloud backend. |
| `fgt74_cmdb_log_get_stats` / `fgt74_cmdb_log_get_current_disk_usage` | Log volume per category/device and disk usage — corroborates whether a category generates data at all. |
| `fgt74_cmdb_log_get_historic_daily_remote_logs` | Daily volume of logs sent to remote backends — evidence of what the remote source should hold. |
| `fgt74_cmdb_log_get_local_report_list` / `fgt74_cmdb_log_get_forticloud_report_list` | Pre-generated local/FortiCloud reports (and `..._download` counterparts). |
| `fgt74_cmdb_search_post_abort_session_id` | Ends a paginated log search session. A POST, but it only terminates your own search session — non-mutating. |
...
1. **`fgt74_cmdb_log_get_device_state` first — never skip it.** It tells you which backends can hold data before you spend calls on ones that cannot.
2. **Pick the backend by evidence**: remote (FortiAnalyzer, then FortiCloud) when device_state shows it connected/enabled; on remote failure or disabled state, fall back to disk (if available) and then memory. Diagnose a failing remote backend with its state anchors (`fgt74_cmdb_log_get_fortianalyzer`, `fgt74_cmdb_log_get_forticloud_connection`) — report the cause, then continue on a local store.
...
4. **Empty result → classify it before concluding** (see Interpreting below). Check `fgt74_cmdb_log_get_stats` to see whether that category generates volume at all.
...
6. If you paginated with `session_id`, close the session with `fgt74_cmdb_search_post_abort_session_id`.
...
- `fgt74_cmdb_log_post_stats_reset` — **indexed and executable**: resets logging statistics on all log devices. Never execute it.
- Blocked from the index but do not propose them blindly in plans either: `fgt74_cmdb_log_post_local_report_delete`, `fgt74_monitor_sys_post_logdisk_format` (destroys all disk logs), `fgt74_cmdb_log_disk_filter_delete_custom_field_id`.
...
- **`fgt74_cmdb_log_disk_filter_*` tools (115 of them) are logging CONFIGURATION** (`/api/v2/cmdb/log.*`) — useful to verify what is being logged and where (including syslog destinations), useless for reading entries.
- **Misleading names**: disk retrieval is `fgt74_cmdb_disk_get_*`, memory retrieval is `fgt74_log_mem_get_memory_*` — do not infer capability from names.
