...
- **An explicit `device` header that resolves to no device is rejected** with a synthetic HTTP 404 (`{"error": "unknown_device"}`) before any appliance is contacted — it never falls back to the primary. The primary fallback exists only for requests that carry **no** `device` header (manual/single-device use). Rationale: a caller that addressed a specific appliance and silently reached a different one executes the whole call chain against the wrong device — for enrichment that means writing another appliance's data onto the asset (cross-device contamination) with nothing but a gateway log line to show for it. App-side, the routing identity is the **asset id** (`assets.id`, the gateway device `id`); the asset `ref`/`name` are human reference fields and never route. `src/core/mcp_executor.py` canonicalizes a caller-supplied `device` (the Engineer LLM may pass `ref`) to the asset id tenant-scoped before dispatch.
...
- After the appliance answers, `send()` **repairs non-UTF-8 JSON bodies**: appliances serialize stored text fields verbatim, so a raw Latin-1 byte (e.g. an accented character in a FortiCare company name — `0xED` = `í`) makes the body invalid UTF-8 and `httpx.Response.json()` inside FastMCP crashes the tool call (`'utf-8' codec can't decode byte …`). Only the invalid bytes are mapped through Latin-1 (a whole-body Latin-1 re-decode would mojibake legitimate multi-byte UTF-8); valid bodies pass untouched, repairs are logged as warnings. Tested in `gateway/tests/test_routing_client.py` + `test_routing_reload.py`.
