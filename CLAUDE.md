...
- **Asset Inventory module** (`src/assets/`): relational MSP asset inventory replacing the Component-in-blob storage. Tables `assets`/`asset_relations`/`asset_definition_versions`/`asset_sync_runs`/`asset_audit_log`/`asset_subitems` (JSONB+GIN — deliberate deviation, Postgres-only; migrations `a8b9c0d1e2f3` + backfill `b9c0d1e2f3a4` + subitems `c0d1e2f3a4b5`); `ContextStore.get_active_context` overlays inventory/dependencies from the tables via `src/assets/context_adapter.py` so engineer/assessments/pack_matching consumers are unchanged; legacy `/inventory` component/dependency endpoints are delegating shims (baselines/known_changes stay blob-owned). Versioned YAML asset-type schemas + enrichment packs (immutable snapshots, assessments registry pattern). Deterministic enrichment (NO LLM): packs fortigate@2/fortiedr@3 over `execute_mcp_tool(enforce_read_only=True)`, per-field provenance manual/discovered (`manual_wins` default). **Curated-vs-discovered invariant is structural**: assets are curated (human/import); discovery lands in `asset_subitems` (kind/state/attributes, upsert by `(customer_id, parent, source, kind, external_id)`, `absent` flag on complete scans, never deleted, no provenance/audit) — the FortiEDR collector inventory is subitems on the console asset, aggregated as `subitems_summary` on API responses and `metadata["subitems"]` in ClientContext; triggers auto/manual/scheduled (`ASSETS_SYNC_INTERVAL_HOURS`, in-process loop + startup sweeper). API `/assets` (20 endpoints incl. `GET /{id}/subitems`; perms `assets:read/write/manage/read_global`; `/assets/global` cross-tenant for platform admin; export CSV/XLSX, import dry-run upsert); frontend `pages/assets/` + GlobalAssetsPage (Sub-inventory tab, per-row kind chips). Safety filter fix: tool-NAME matching is token-based (unblocks `fgt74_monitor_sys_get_vm_information` + 2 autoupdate GETs; newly blocks all `*_put_*` writers + `vmlicense` POSTs — re-index `tool_catalog` after deploy). Docs: `docs/assets.md`, `docs/assets_enrichment.md`
